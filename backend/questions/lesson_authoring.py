...
# backend/questions per rule 2 -- backend/lessons (the pure Lesson model +
# orchestration) never calls an LLM itself, exactly the same split
# backend/curriculum already keeps from backend/research (rule 16).
#
...
_SYSTEM_PROMPT = """\
You are composing one lesson explanation for a course, from a fixed set of \
...
Your job: write a clear, well-organized EXPLANATION of the concept, built \
ONLY from the claims given. You may synthesize, reorder, and connect the \
claims into readable prose, and you may draw the most basic, unavoidable \
connective inferences a reader would need to follow the explanation \
sentence-to-sentence (e.g. "therefore", "as a result") -- but you must NOT \
introduce a new fact, example, mechanism, number, or named entity that is not \
present in the claims given, even if it is common knowledge to you and even \
if it would make the explanation better.

If the claims are too thin, contradictory, or fragmentary to produce a \
coherent explanation, say so plainly in the explanation itself rather than \
inventing content to fill the gap -- an honest, incomplete-sounding \
explanation is correct behavior here, not a failure.
