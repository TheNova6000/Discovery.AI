...
## 2026-09-17 — Coverage contract formalized: `backend/dewey/coverage/`, the checker renamed, negation/collision fixes measured, and a full-suite regression check finding no breakage

Direct continuation of Experiment C below, per explicit instruction: "The next task should be neither Experiments A/B nor immediate LLM integration. It should be: Write the coverage contract and rename/reframe the current checker as lexical candidate detection." Checked against Phase 8.3's existing `FieldCoverage`/`ConceptCompleteness` before writing code -- confirmed a genuinely different axis (per-entity internal field completeness vs. cross-topic external concept presence), not a duplicate.

**New `backend/dewey/coverage/` package:** `schema.py` (the `CoverageStatus` ontology -- `ABSENT`/`MENTIONED`/`DEFINED`/`EXPLAINED`/`EXEMPLIFIED`/`BOUNDED`/`CONTRASTED`/`MISCONCEPTION_CORRECTED` -- with a `model_validator` that structurally prevents any `"lexical"`-named checker from constructing a result claiming a semantic status, plus a `.semantically_verified` property so downstream code never has to infer "covered" from `status != ABSENT` itself); `benchmark_cases.py` (the 27 hand-labeled cases from Experiment C, persisted permanently per explicit instruction, no longer inline in a throwaway script); `lexical_candidate_checker_v0.py` (the renamed checker, honestly scoped in its own docstring to only ever output `ABSENT`/`MENTIONED`).

**Two real, benchmark-driven fixes:** clause-based negation detection (splitting text on `.`/`;`/`but` so "explains new... but does not cover smart pointers" correctly separates into an affirmed clause and a negated one, instead of one global check wrongly suppressing both) and per-concept domain-collision exclusion phrases (a small `DEFAULT_EXCLUSION_PHRASES` map fixing the Python-GC-vs-C++-"reference" adversarial false positive from Experiment C).

**Real, measured result: precision 0.48 → 0.59, recall held at 1.00, true negatives 0 → 5/27.** A genuine regression was caught mid-build, not after: an initial strict `\bphrase\b` + optional-`s` pattern dropped recall to 0.92 by failing to match `EXPECTED_CONCEPTS`' deliberate partial-word stems (`"dereferenc"` needs to match "dereferencing"); fixed by anchoring the word boundary at the start of the phrase only, with a trailing `\w*`. The remaining 9 false positives are structurally expected, not bugs -- 8 are mention-only/shallow cases where `MENTIONED` is the lexically honest answer to a question the ground truth was actually asking semantically; the 9th is a real, named cross-clause negation limit deferred to a future semantic checker.

**`scripts/verify_coverage.py`** (new, real pass/fail regression test, not diagnostic) asserts precision ≥0.59/recall ≥1.00 against the persisted benchmark. `scripts/evaluate_source_pack_quality.py` now imports the shared checker instead of keeping its own copy. `scripts/evaluate_coverage_checker_precision.py` deleted -- fully superseded by `benchmark_cases.py` + `verify_coverage.py`.

**Regression check against the rest of the system, prompted by an explicit external-review concern** that the afternoon's retriever/checker changes could have broken previously-working Discovery.AI behavior, not just measured the checker's own weakness: the full 47-script `scripts/run_all_verifications.py` suite was run twice (first run's summary counts were accidentally lost to a `tail -80` truncation on the invoking side -- a real self-caught mistake, not a suite problem -- then re-run with output correctly captured to a file). **Real result: 46 passed, 0 environment-blocked, 0 timed out, 1 failed.** The one failure, `verify_phase5.py`, was re-run standalone and its real cause fully captured in its own output: a Groq `429 rate_limit_exceeded` on every one of the 9 configured keys across all 4 organizations, each reporting `Used ~199,4XX-199,9XX / 200,000` tokens for the day -- genuine, same-day quota exhaustion from this session's own cumulative live-LLM testing, not a code defect. `run_all_verifications.py`'s own `environment_blocked` detection only recognizes a *missing* API key, not a 429 against a present one -- a real, minor categorization gap, named here rather than left to silently misreport a quota event as a code "failure." **No evidence found of a registration-scope regression:** adding `CppReferenceRetriever`/`GitHubRetriever` to `DEFAULT_RETRIEVERS` did not break any of the 46 unrelated, previously-passing scripts spanning topics that predate the Source Pack work entirely.
...
**`MasterAgent.run_task_graph(tasks, task_questions, *, actor, task_budget) -> TaskGraphResult`** is a new method, new `TaskGraphState`, new LangGraph graph with its own checkpoint thread — `run()` and `MasterState` have zero lines changed. Per round: unlock newly-runnable blocked tasks (R3.1's `compute_runnable_tasks`); detect duplicates against already-`complete`/`running` tasks and short-circuit them straight to `complete` (copying the canonical's `result_refs`); apply `task_budget` (an independently-settable concept, defaulting to `spawn_budget` but not the same thing); transition the batch to `running`; run real `GroundAgent`s via `asyncio.gather(..., return_exceptions=True)`; transition each to `complete`/`failed` from its real outcome. The round-robin repetition is a plain Python loop inside ONE checkpointed LangGraph node in this slice, stated explicitly as a scoping choice (not a graph-level conditional-edge cycle) — a safety cap (`len(tasks)+1` rounds) guarantees termination regardless.

**Two new pure functions in `backend/reasoning/tasks.py`, same zero-cross-package-import discipline as R3.1:** `validate_task_graph` (DFS cycle detection + unknown-dependency rejection, raised eagerly before any LangGraph state exists) and `compute_tasks_blocked_by_failed_dependency` (a real distinction the acceptance tests forced into the open: a merely-`"failed"` dependency might still retry-succeed, so its dependent correctly stays ordinary `"blocked"`; a `"budget_exhausted"` dependency is permanently dead, and that fact is explicitly, separately reported).

**A real design consequence of R3.1's own contract, worked through rather than loosened:** `ResearchTask` still carries no `Question` field (adding one would break `backend.reasoning`'s zero-import-from-`backend.questions` guarantee). `run_task_graph`'s `task_questions: dict[str, Question]` parameter supplies the payload at the orchestration layer instead — `backend/agents/models.py`'s new `TaskGraphResult` is the first file in `backend.agents` to import from `backend.reasoning`, confirmed a safe, one-way addition (no cycle).

**`AgentStatus.BOUNDARY_HIT` is deliberately treated the same as `COMPLETE`** for a task's outcome — not invented for this slice, but exactly `scripts/verify_phase4.py`'s own existing convention. Escalating a task-graph boundary hit is Phase 7 territory, explicitly out of scope, not silently absorbed.

**Verified: `scripts/verify_r3_2.py`, 11/11 deterministic checks** (`decide_next_step` monkeypatched exactly as `verify_phase4.py` already does — real MasterAgent, real LangGraph, real GroundAgent, zero LLM/retriever/Neo4j calls), **plus one optional live-provider smoke reported separately and skipped in this environment** (no key configured, per Rule 14 — never folded into the 11). Covers the exact DNS A/B/C acceptance example end-to-end: initial split, dependency unlock, partial completion, duplicate detection, missing-dependency and cycle rejection (both at `validate_task_graph` and at `run_task_graph` itself), failed-vs-exhausted dependency handling, budget exhaustion (`task_budget=1` executes exactly one task, terminates in 2 rounds, no infinite loop), a raising GroundAgent recorded `"failed"` with its real error preserved, idempotent re-entry (0 additional LLM calls / 0 additional executed tasks on a second call), and the full graph completing in 3 rounds through the real orchestration path. All 79 pre-existing checks re-confirmed unaffected.

**Known limitations recorded, not hidden:** two simultaneously-already-runnable duplicates in the same round both execute independently (suppression only guaranteed once one reaches running/complete); `BOUNDARY_HIT` escalation deferred; re-entry idempotency verified at the scheduler's data level, not via checkpointer-thread resumption across separate calls.

**R3 is now fully built across both slices (R3.1 + R3.2).** Per the prompt's own explicit non-goals, claim/subclaim orchestration (R4) and the Research API (R5) are not attempted in this pass.

---

## 2026-09-16 (continued, same day) — R3 built (first slice): the ResearchTask domain type; the MasterAgent evaluation confirmed "resurrect and extend" against the real file, not a leaning accepted on faith

Follows R2, same session, per the user's "loop until phase 3... rectify updates after phase 3 with different tests" instruction — read as: finish R3 with real evaluation + real tests, defer further wiring/polish to a later pass.

**The MasterAgent evaluation ran first, against the real `backend/agents/master_agent.py`, not from memory of the earlier Phase 8.1 investigation of it.** Confirmed what generalizes: `enforce_spawn_budget`'s "commit to a selection, checkpointed, before spawning" LangGraph-node shape (same node position works for a richer, dependency-aware eligibility computation) and `spawn_and_run`'s bus+`GroundAgent`-per-batch spawn mechanics. Confirmed what does NOT survive unchanged: `enforce_spawn_budget`'s actual body (a flat `questions[:budget]` slice with zero dependency awareness) and `MasterState`'s one-shot `TypedDict` shape, both needing real replacement/extension. **Conclusion: resurrect and extend — confirmed by reading the code, not accepted as the design pass's own leaning.** Recorded as a new §0.54.1 addendum, same "dated correction/confirmation, not silent edit" pattern as R2's §0.52.1. A decisive extra fact: `MasterAgent` still has zero live `/chat`-path callers, so R3's work carries zero live-traffic regression risk — better than Phases.md's original "medium-high" risk framing assumed.

**First slice, scoped like R1.1 (types + invariants first, no orchestration wiring yet):** `backend/reasoning/tasks.py` — `ResearchTask` (§0.53's shape: task_id, investigation_id, target_entity_id, research_field, task_type, dependencies, status, attempt_count/max_attempts, result_refs) plus four pure functions. `initial_status` derives blocked/runnable structurally from whether a task has dependencies. `compute_runnable_tasks` requires *all* dependencies complete, not just one of several (explicitly tested — the case where a naive check would wrongly unblock early). `detect_duplicate_task` compares structural identity (investigation/entity/field/type), the same pattern as R1.3's `identity_floor` applied to tasks — the concrete fix for Phase 6/8.6's real duplicate-Question/duplicate-Claim problem — and correctly ignores a `budget_exhausted` prior so a dead task never blocks a genuine retry. `transition_task` mirrors R1.4's `transition_claim` exactly: mandatory reason/actor, an enforced legal-transition graph, retry only legal while attempts remain, reconstructed through the real constructor so the model's own validator stays the source of truth. One deliberate split, stated explicitly: blocked/runnable are structural (graph-derived, no reason/actor needed); running/complete/failed/budget_exhausted are execution outcomes an actor asserts, and the model requires `provenance_note` for all four — same "status is an asserted fact" principle as `Claim`.

**Deliberately not built this slice:** any change to `master_agent.py` itself (the node bodies, the scheduling cycle, `MasterState`'s extension) — the evaluation identified exactly what that requires, but wiring it in is separate, later R3 work, mirroring R1.1 → R1.2's "types first, wire second" precedent.

**Verified: `scripts/verify_r3_1.py`, 10/10, pure, no I/O.** Initial-status derivation; multi-dependency runnable computation; duplicate detection including the budget_exhausted-ignored case; every legal/illegal transition edge; empty reason/actor rejection; the real retry-then-exhaustion path; round-trip serialization (fully-populated and sparse); zero cross-package imports; the status-consistency invariant firing for all four execution statuses. All 76 pre-existing checks re-confirmed unaffected.

**R3's first slice is complete as scoped.** Per the user's own instruction, further R3 work (wiring `ResearchTask` into `MasterAgent`'s live LangGraph) is deferred to a later pass with its own tests, alongside R4/R5 which remain unstarted.
...
**Phase 6.1: closed the gap Phase 6's own live verification found (that entry's point 2) — a real user had no path to a working `abstraction_id`.** Renamed `backend.graph.interface.zoom_in` → `materialize_abstraction` (the old name's collision with `app.py`'s unrelated `handle_zoom_in` is what let a zero-caller function hide through all of Phase 6's build-and-verify pass — a distinct name makes that specific failure mode structurally harder to repeat). Built two real entrypoints reusing it: `POST /roadmap/build` (REST, the primary path) and a new `"build_roadmap"` chat intent (convenience path, "build me a learning roadmap for this" → a real markdown link in the reply). Added a third button, "Build Roadmap," to `chat.html`'s existing per-message options panel (next to "Resources"/"Regenerate") rather than inventing a new UI surface. Verified via curl against the real local backend + real Aura Neo4j: both the REST endpoint and the chat intent correctly produce the *same* idempotent `abstraction_id` Phase 6's manual verification had already produced for "online payment," and the "no decomposition yet" failure path correctly 400s. **Found one more real bug this way, not anticipated in advance:** `handle_build_roadmap` didn't set `session.current_entity`, so the "Build Roadmap" button on the reply's own chat message had nothing to act on — confirmed live via direct DOM inspection (`wrap.dataset.entityName === ""`), fixed to match `handle_zoom_in`'s existing pattern. Full browser click-through-to-new-tab wasn't cleanly completed this session — `chat.html`'s `CONFIG.BACKEND_URL` is hardcoded to the real deployed production backend rather than defaulting same-origin the way `roadmap.html`'s own `CONFIG` does, and overriding it at the console collided with a real cached Supabase login already present in the test browser profile. Real local-dev friction, not a defect in the shipped path — flagged, not fixed, since it means touching working production config.

**Learning Research Mode: the bigger, genuinely architectural correction, triggered by what that live roadmap actually looked like.** 9 real steps, correctly ordered — but all `ground`-level, zoom-depth 0-2, most resources at 10-15% confidence. Not a Phase 6 defect: the base engine's default investigation policy showing through exactly as designed (one question per entity, one evidence pass, decomposition bounded unless a user explicitly says "go deeper" — the right default for bounded-cost exploratory chat). The user's read, confirmed correct on inspection: Discovery.AI's objective ("investigate enough to build a dependency/structure graph") and the Learning Portal's actual objective ("investigate enough to teach the topic correctly and deeply") are genuinely different stopping conditions, not the same policy at two depth settings — satisfied by structure alone vs. requiring real per-concept content (definition, mechanism, example, misconceptions, evidence meeting a confidence bar) before something counts as "researched."

**Explicitly rejected as the fix: neither "make `GroundAgent` deeper everywhere" (blows up cost/latency for ordinary chat) nor "have the Learning Portal repeatedly call `investigate_deeper` as an external workaround" (fragile — forces the research engine to behave differently through a sequence of manual commands from outside, rather than the engine itself understanding what "enough" means for the objective it's serving).** Decided instead: two modes, one engine — `exploratory` (today's existing behavior, unchanged) and `learning` (new), both reusing the same `GroundAgent`/Evidence Engine/Graph Interface/key-pool, differing only in an orchestration policy layered on top of the same machinery. Also decided, as a hard boundary: research completeness ("is this concept sufficiently understood and evidenced," the new mode's job) and learning completeness ("can this become an effective lesson — sequencing, misconception-preemption, exercise design," the Curriculum Compiler's unchanged job) stay two separate concerns — the research engine is never made directly responsible for producing the final course.

**This was a documentation-only session for Learning Research Mode, deliberately** — no `ResearchPolicy`/planner/completeness-model code exists yet. Written into PRD.md §9.3a (the spec: the two-mode split, the per-concept completeness fields, the `ResearchPolicy` field sketch, the research-vs-learning-completeness boundary), Architecture.md §0.40 (the reasoning and what was explicitly rejected), and Phases.md (Phase 8 restructured from one flat "Curriculum-source retrievers" phase into 8.1-8.6 — Research Policy abstraction, Learning Research Planner, Coverage/completeness model, Deep investigation orchestration, Evidence/contradiction validation, Research-complete graph artifact — with the original retriever work repositioned into 8.4 as infrastructure serving the new orchestration, not lost). Phase 9's Curriculum Compiler entry updated to consume Phase 8.6's research-complete artifact instead of a bare exploratory subgraph, with a corresponding new functional requirement in PRD.md §9.6 (a concept failing the completeness bar must be surfaced as incomplete, never silently compiled into a thin module).

**Next up:** Phase 8.1 (the `ResearchPolicy` type + an `exploratory`-preset regression guard proving it reproduces today's behavior exactly) is the actual first Phase 8 implementation step — not retrievers, per this session's own correction. Phase 6.1's one remaining gap (full browser click-through verification, and `chat.html`'s hardcoded-production-URL local-dev friction) is worth a clean pass before or alongside that, but doesn't block it.

---

## 2026-09-16 — Phase 6 started: `generate_roadmap` built and unit-verified; Learning Portal docs extended; two real doc gaps fixed

First Learning Portal implementation session (not just docs) — Rules.md's "don't skip writing Memory.md once implementation starts" applies from here on for this track.

**Docs first (same session, chronological order this actually happened in):**
1. Cloned this repo into a fresh local working directory (the user chose "fork/extend Discovery.AI itself" over a separate service/library when asked directly) and extended PRD.md/Architecture.md/Rules.md/Phases.md/Design.md with a Learning Portal scope (§9, §6, rules 15-20, Phases 8-13) — additive only, nothing in the base engine's existing scope touched.
2. User asked for the comma-separated multi-API-key feature "from Discovery.AI" to be added. **Investigated first, found it already existed and was already live** — `backend/questions/llm_config.py`'s `_collect_keys()`/`PROVIDER_KEY_POOLS`, round-robined in `llm_client.py`, plus a request-scoped bring-your-own-key override. It had simply never been written into PRD.md/Architecture.md/Rules.md. Documented it (Architecture.md §0.38.1) rather than re-implementing something that already worked, and made it the required pattern for any new keyed integration (Rules.md rule 21) — a real doc-gap fix, not new code.
3. Researched real gamified/coding-education platforms (Duolingo's XP/streak-with-freeze/skill-tree, Codewars' per-kata kyu rank, Exercism's mentor-quality feedback framing, LeetCode/HackerRank's difficulty tiers, and a paper on gamification actively harming learning when it fabricates progress — arXiv:2203.16175) before adding a gamification layer to PRD.md §9/Architecture.md §0.38.3/Design.md/Rules.md rule 22 — grounded in what real products do and what's documented to go wrong, not invented from scratch.
4. User clarified local-first is a **development-stage** target, not a permanent architectural ceiling — the plan already left a cloud path open (§5/§6.5), this was made explicit (Architecture.md §0.38.2's addendum) rather than changing any actual decision.
5. A pasted external summary of the project proposed a new "Phase 7 — Integration foundation." **It was wrong that Phase 7 was free** — Phase 7 already belongs to the base engine's Abstraction-change-protocol/conflict-resolution work (Phases.md, pre-existing, unrelated to the Learning Portal), which is exactly why the Learning Portal track was numbered 8-13 in the first place. Corrected this and folded the summary's actual proposed content (job lifecycle, API contracts, retries, cost/key-usage logging) in as "Post-Phase-6 — Integration foundation," an unnumbered ad hoc section, matching the existing "Post-Phase-5 — Epistemic layer" precedent rather than renumbering anything.

**Then real Phase 6 implementation** — the user pushed back on "more architecture brainstorming" and asked to actually build. What got built:
- `backend/roadmap/` (`models.py`: `Roadmap`/`RoadmapStep`; `ordering.py`: `order_roadmap_steps` + `_compute_zoom_depths`, pure functions, zero I/O; `generator.py`: `generate_roadmap(abstraction_id) -> Roadmap`, the async Graph-Interface-only shell around the pure logic). Decided to give this its own module rather than `backend/questions` (Architecture.md §0.39.1) since it has zero dependency on that package.
- `GET /roadmap?abstraction_id=...` added to `backend/api/app.py`, mirroring `/resources`/`/node_detail`'s existing shape (plain dict return, 404 via `GraphInterfaceError` on a bad id).
- `frontend/roadmap.html` — vanilla JS/HTML/CSS (no build step, matches the real live frontend stack, not the originally-planned-but-unbuilt React path — Architecture.md §0.38.3), served automatically by the existing `StaticFiles` mount. Takes a raw `abstraction_id`; topic-name resolution is a known, flagged gap, not built yet.
- `scripts/verify_phase6.py` — 5 checks against the pure ordering logic (basic master-before-ground/zoom-depth ordering against a small hand-built graph; a pure cycle terminates and every node still gets a depth; a question attached to an entity outside the given subgraph is skipped rather than invented a place for; calling the function twice on identical input is deterministic; an unrecognized `level` value degrades gracefully instead of raising). **Actually run, all 5 pass** — real output, not assumed.

**Real environment constraint, not glossed over:** this session's shell had no `docker` binary and no `neo4j` Python package installed. `pip install neo4j` was run just to make `backend.graph`/`backend.roadmap` importable for the pure-logic test (that import chain pulls in `backend/graph/driver.py`, which imports `neo4j` even though the test itself never opens a connection). No live Neo4j was reachable, so `generate_roadmap`'s actual database round-trip (`get_subgraph`/`get_questions_for_entity`/`get_claims_for_question`) was never exercised, and neither was the new `/roadmap` endpoint or `frontend/roadmap.html` against a real backend. Phases.md marks Phase 6 `[PARTIAL]`, explicitly not `[VERIFIED]`, until that live run happens — on a machine with Docker and `pip install -r requirements.txt` available, most likely the actual laptop this is meant to run on.

**Live verification actually completed later the same day (Architecture.md §0.39.2, Phases.md Phase 6) — no Docker needed.** Pointed `.env` at a free Neo4j Aura instance (README.md's documented alternative), ran `pip install -r requirements.txt` (the earlier `pip install neo4j` alone wasn't enough — `uvicorn` failed on `langgraph` until the full file was installed), and ran a real investigation.

**Found and fixed a real bug along the way, not just a verification formality.** The first live investigation crashed with `"Something went wrong investigating that: 'charmap' codec can't encode character '‑'..."` — a genuine bug, not free-tier flakiness: Windows' `cp1252` console can't print a Unicode non-breaking hyphen, and an unsanitized `print(f"...{exc}")` in `backend/agents/ground_agent.py` (one of ~14 such call sites across `ground_agent.py`/`app.py`/the evidence retrievers — only `llm_client.py`'s own logging had ever been sanitized, after an earlier documented incident) crashed trying to echo an LLM's raw Unicode output back through an exception message. Fixed at the root in `backend/__init__.py` (reconfigures `sys.stdout`/`sys.stderr` with `errors="backslashreplace"` at import time — covers every call site, present and future, plus third-party libraries' own logging) rather than patching each print individually. Re-ran the same investigation after the fix; it completed successfully with the identical Unicode content still present in the answer, confirming the fix rather than a lucky retry.

**Found, and deliberately left open, a real product gap: nothing in the live app can ever produce a working roadmap link.** `/roadmap`'s `abstraction_id` refers to a graph `Abstraction` node, and the only function that creates one — `backend.graph.interface.zoom_in()` — has zero callers anywhere in `app.py`. The chat `"zoom_in"` intent is wired to a *different*, same-named function (`app.py`'s `handle_zoom_in`) that does pure navigation and never touches `Abstraction` nodes at all — confirmed by grepping the whole codebase for real callers. This session only reached a working `abstraction_id` by invoking `backend.graph.interface.zoom_in()` directly via a one-off script, which no real user or the frontend can do. This is now the actual next blocker for Phase 6 usability (not the already-known topic-name-resolution gap — that has nothing to resolve *to* until this is wired up first) — left open rather than fixed because which chat action should trigger it is a real product decision, not a mechanical fix.

**With those two findings accounted for, the full live path was verified and passed:** real `/chat` investigation (41 nodes/55 edges) → `zoom_in()` → real `abstraction_id` → `GET /roadmap` → 9 steps correctly ordered by `(level, zoom_depth)` → `frontend/roadmap.html` loaded in an actual browser via `claude-in-chrome`, rendered correctly, zero console errors. First real (not fixture-based) confirmation anywhere in the Learning Portal track. Two smaller, out-of-scope-for-Phase-6 observations also surfaced and were logged for later: duplicate `Question` nodes for the same entity/text across retry passes (no dedup, unlike `find_or_create_entity`), and generally low (10-15%) resource-confidence scores reflecting real evidence-retrieval weakness on this topic, not a roadmap bug.

**Next up, before Phase 8:** wire something in the live app to actually call `zoom_in`/`create_abstraction` (the real blocker above) — only after that does "topic-name → abstraction-id resolution" have anything to resolve to. Also worth a look whenever someone's nearby: the Question-node dedup gap noted above.
