...
2. **Cardinality** — one parent per claim, many children per parent (a tree, not a general graph). Deliberately chosen over many-to-many: the real research workflow's own decomposition (`GroundAgent`'s sequential sub-question investigation) is already tree-shaped, and a single `Optional[str]` field is the smallest representation that matches it. A claim genuinely needing multiple independent parents (e.g., one piece of evidence supporting two unrelated claims) is out of scope — a real, stated limitation, not an oversight.
